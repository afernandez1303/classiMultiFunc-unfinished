...
mHausdorff = function ( data , parallel = FALSE , cl = NULL )
{
  ...
  combinations = expandgrid2 ( matrix ( rep ( 1 : n , times = 2 ) , 
                                        ncol = 2 ) )
  if ( parallel == FALSE ) {
    return ( array ( apply ( combinations , 1 , function ( x ) 
      pracma :: hausdorff_dist ( data_array [ , x [ 1 ] , ] , 
                                 data_array [ , x [ 2 ] , ] ) ) ,
      dim = c ( n , n ) ) )
  } else {
    cl = parallel :: makeCluster ( cl )
    parallel :: clusterExport ( cl , 
                                list ( "expandgrid2" , "data" , "data_array" ) ,
                                envir = environment ( ) )
    parallel :: clusterEvalQ ( cl , library ( pracma ) )
    return ( array ( parallel :: parApply ( cl = cl , 
                                            X = combinations , 
                                            MARGIN = 1 , 
                                            function ( x ) pracma :: hausdorff_dist ( data_array [ , x [ 1 ] , ] ,
                                                                                      data_array [ , x [ 2 ] , ] ) ) ,
                     dim = c ( n , n ) ) )

...
mmean = function ( data , parallel = FALSE , cl = NULL )
  ...
  n = unique ( sapply ( data , 
                        ncol ) )
  if ( parallel == FALSE )
    {
    return ( matrix ( apply ( cbind ( 1 : n ) , 
                              1 , 
                              function ( i ) sqrt ( colSums ( ( t ( sapply ( data ,
                                                                             function ( x ) colMeans ( x ) ) ) - 
                                                                t ( sapply ( data , 
                                                                             function ( x ) colMeans ( x ) ) ) [ , i ] ) ^2 ) ) ) ,
                      ncol = n , nrow = n , byrow = TRUE ) )
    } else {
      cl = parallel :: makeCluster ( cl )
      parallel :: clusterExport ( cl , 
                                  list ( "data" ) )
      return ( matrix ( parallel :: parApply ( cl , 
                                               cbind ( 1 : n ) , 
                                               1 , 
                                               function ( i ) sqrt ( colSums ( ( t ( sapply ( data , function ( x ) colMeans ( x ) ) ) - 
                                                                                 t ( sapply ( data , function ( x ) colMeans ( x ) ) ) [ , i ] )^2 ) ) ) ,
                        ncol = n , nrow = n , byrow = TRUE ) )
